...
// temporary set to 8 hours, should be cleaned up at some point
...
app.post("/start-ai-agent", async (req, res) => {
    ...
    const user_id = `ai_bot-${channel_id.replace(/[!]/g, "")}`;
...
app.post("/token", async (req, res) => {
    try {
        ...
        const expiration = issueAt + 60 * 60; // 1 hour from now

        const token = serverClient.createToken(userId, expiration, issueAt);
